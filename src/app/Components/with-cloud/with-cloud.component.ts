...
@Component({
  selector: 'app-with-cloud',
  templateUrl: './with-cloud.component.html',
  styleUrls: ['./with-cloud.component.scss']
})
export class WithCloudComponent implements OnInit {
  modalRef!: BsModalRef;
  topologyArr: any;
  ...
  ngOnInit(): void {
    this.dashSer.getData().subscribe((res: any) => {
      ...
      this.width = Math.max(maxLenghtDevices * (this.deviceSize * 2), 400) * .9;
      this.setNodeLinks();
    });
  }
  ...
  setNodeLinks() {
    ...
    console.log(this.duCellsArr.flat().map((d) => d.latitude + ',' + d.longitude));
